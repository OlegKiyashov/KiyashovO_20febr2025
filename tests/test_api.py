...
@allure.description("Добавляем товар в корзину и проверяем корректность ответа API.")
@allure.id(1)
...
@allure.description("Добавляем товар в корзину, затем удаляем его и проверяем, что корзина пуста.")
@allure.id(2)
@allure.severity("Blocker")
...
@allure.description("Добавляем товар в корзину, затем изменяем его количество и проверяем корректность изменения.")
@allure.id(3)
@allure.severity("Blocker")
def test_change_amount_product(api):
    ...
    with allure.step("Проверяем корректность изменения количества товара"):
        assert status_code_change == 200, f"Ожидался статус-код 200, но получен {status_code_change}"
        assert result_change["status"] == "ok", (
            f"Ожидался статус 'ok', но получен {result_change['status']}"
        )
        expected_amount = amount_before_change - 1
        assert amount_after_change == expected_amount, (
